...
class Period(models.Model):
    ...
    description = models.TextField()
    sorting_order = models.TextField()
...
class Piece(models.Model):
    ...
    # many-to-one - many pieces to one composer - a piece can have one composer - one composer can have many pieces
    composer = models.ForeignKey(Composer, on_delete=models.CASCADE)
    # many-to-one - many pieces to one period  - a piece can have one period - one period can have many pieces
    period = models.ForeignKey(Period, on_delete=models.CASCADE)
    # many-to-many - one piece can have many techniques, one tecnique can have many pieces
    techniques = models.ManyToManyField(Technique)
    tempo = models.ManyToManyField(Tempo)
    difficulty = models.IntegerField()
    # one piece can have many prerequisites, each prereq can have many pieces
    ...
    # many-to-one - each piece can only have one type, each type can have many pieces
    type_of_piece = models.ForeignKey(TypeOfPiece, on_delete=models.CASCADE)
    ...
    def sorting_title(self):
        ...
        for i, char in enumerate(self.title):
            if char.isdigit() and start == -1:
                ...
                if not self.title[len(self.title) - 1].isdigit():
                    break
        if start == -1:
            return self.title
